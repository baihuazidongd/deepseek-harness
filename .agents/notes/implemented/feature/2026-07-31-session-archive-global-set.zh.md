...
归档后数据与 slot 完好。暂缓的恢复面已作为逆向 RPC + 设置页落地，见 [2026-08-23-session-unarchive-recovery](2026-08-23-session-unarchive-recovery.md)。`workspace.list` 响应形状变化是 pre-release 直改（无兼容层）。e2e（workspace-management）钉住了「归档→行消失→reload 后仍隐藏、日志仍在」的全链路；domain 层测试钉住幂等、未知 id 拒绝、跨重启恢复与旧介质默认升级。
