...
Archived sessions keep their data and accounting slots intact. The deferred restore surface shipped as the inverse RPC plus Settings page in [2026-08-23-session-unarchive-recovery](2026-08-23-session-unarchive-recovery.md). The `workspace.list` response shape change is a pre-release direct edit (no compatibility layer). The workspace-management e2e pins the full chain (archive → row disappears → still hidden after reload, log still present); domain tests pin idempotence, unknown-id rejection, restart recovery, and the pre-field media default upgrade.
