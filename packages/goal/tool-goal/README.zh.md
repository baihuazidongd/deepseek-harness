...
- `create_goal(objective, max_goal_rounds?)` 从任意顶层轮次创建一个 goal。模型识别到长期运行目标时即可自动创建，无需精确命令短语；执行时仅拒绝 subagent。
...
执行要求完全相同的活跃 `exec.agent`、其继承的 `AgentRegistry` initiator、running 状态与开放轮次。`create` 还要求运行时根 agent，因此任意顶层轮次——直接人类请求、goal 轮或插件续作轮——都能自动创建 goal；活跃 subagent 会被拒绝。`edit`、`pause` 和 `resume` 还要求运行时根 agent 的当前轮次中存在已接受的 `{ kind: 'user' }` 消息或 steering 事件。持久 fork 谱系不会降低已恢复根 agent 的等级；活跃 subagent 所有权会降低。
...
固定 goal 策略指示模型在识别到长期运行目标时自动创建 goal，要求更新前先精确读取 ref，解释会话 resume／fork 后如何重新启用续行，并限制完成／阻塞声明。配置的阈值会插入该指引。
...
- **语义意图仍由模型判断**：执行只能证明调用方是顶层 agent，无法证明请求是否足够重大而值得创建 goal。
