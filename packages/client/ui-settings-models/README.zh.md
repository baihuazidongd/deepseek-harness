...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口、输出上限与思考强度档位收在该行的展开区内，右侧是两个无文字的操作——展开与删除。思考强度字段以逗号分隔的规范档位 ID（`off`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`）录入，并作为该模型的 `reasoningEfforts` 映射存下，因此输入框的模型选择器会为该模型提供这些档位；未知档位会被拒绝并点名，档位集合不变时保留已存的协议拼写。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。
