...
- `ctx.workspaceRegistry.unarchiveSession(id)` — removes one id from the archive set and answers the full updated set; an id not currently archived (including an unknown one) resolves without writing. Accounting is never touched.
