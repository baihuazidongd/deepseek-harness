...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-mods': { kind: 'none', reason: 'Browser-side management surface over Host composition state; each toggled plugin owns its own model-facing effect.' },
  'packages/client/ui-slg': { kind: 'none', reason: 'Browser-side conversation surface; renders logged snapshots and forwards input, registering nothing model-facing.' },
  ...
  'packages/boot/user-patches': { kind: 'none', reason: 'Launcher-fact plumbing and a pure patch-file writer; registers nothing model-facing.' },
